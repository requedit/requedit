use std::{fs, path, io::Error, sync::Mutex};
use serde_yaml;
...
pub(crate) fn load() -> Result<Config, Error> {
    ...
    if !workplace_path.exists() {
        fs::create_dir_all(workplace_path).unwrap();
    }
    let config_path = path::Path::new(&config_name);
    let config = if config_path.exists() {
        let file = fs::File::open(config_path)?;
        let config_from_file: Config = serde_yaml::from_reader(file).unwrap();
        config_from_file
    } else  {
        let default_config = Config::default();
        let file = fs::File::create(&config_name)?;
        let yaml_config = serde_yaml::to_value(&default_config).unwrap();
        serde_yaml::to_writer(file, &yaml_config).unwrap();
        default_config
    };
    Ok(config)
}
